...
export default function ServiceContainer() {
  ...
  const [onSubmitLoading2, setOnSubmitLoading2] = useState(false);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await updateMainServicepage(header);
      if (res.message === "Service Page Content updated successfully") {
  ...
  const handleSubmit = async () => {
    setOnSubmitLoading2(true);
    try {
      const res = await createSingleService(
        header1,
        bannerImage,
        descriptionText,
        keys
  ...
  return (
    <div className="pl-[300px] pt-12 pb-20 px-[3%]">
      ...
      {services.map((ser, index) => (
        <section className="w-full pl-7 mt-10">
          <div className="border border-[#F4F4F4F4] relative rounded-[10px] p-10 pb-6 h-full">
            <div className="absolute top-[-17px] left-[150px] border bg-white border-[#f4f4f4]/95 rounded-full py-[5px] px-[30px] text-sm font-clashmd">
              {`Ser ${index + 1}`}
            </div>
            {/**Form */}
            <div className="grid gap-7">
              <div className="grid gap-3">
                <label className="text-sm font-clashmd">Header 1</label>
                <div className="bg-[#f4f4f4] text-sm rounded-[10px] h-[61px] p-5 focus:outline-none">
                  {" "}
                  {ser?.header}
                </div>
              </div>
              <div className="grid gap-3">
                ...
                <div className="bg-[#f4f4f4] text-sm resize-none rounded-[10px] min-h-[89px] p-5 focus:outline-none">
                  {ser?.descriptionText}
                </div>
              </div>
              <div className="grid gap-3">
                ...
                {ser?.bannerImage ? (
                  ...
                  <div className="bg-[#f4f4f4]/50 relative rounded-[10px] h-[255px]"></div>
                )}
              </div>
            </div>
            <button className="bg-primary hover:bg-white hover:text-primary transition-all duration-150 border-2 border-primary mt-14 mx-auto rounded-full text-base font-clashmd text-white w-[191px] h-[53px] flex items-center justify-center">
              <Link href={`/services/${ser._id}`}>Edit service</Link>
            </button>
          </div>
        </section>
      ))}
      <section className="w-full pl-7 mt-10">
        <div className="border flex flex-col items-center justify-center border-[#898787F4] border-dashed min-h-[150px] relative rounded-[10px] p-10">
          <div className="absolute flex items-center py-2 top-[-21px] left-[150px] border bg-white border-[#f4f4f4]/95 rounded-full px-[30px] text-sm font-clashmd">
            Ser
            {isAdd && (
              <button
          ...
          {/**Form */}
          {isAdd && (
            <main className="pb-20 min-w-full">
              ...
              <section className="w-full mt-10">
                <div>
                  {keys.map((key, keyIndex) => (
                    <section
                      className="w-full pl-7 mt-10"
                      key={key._id || keyIndex}
                    >
                      <div className="border border-[#F4F4F4F4] relative rounded-[10px] p-10 h-full">
                          ...
                          {key.contents.map((content, contentIndex) => (
                            ...
                            >
                              ...
                              <div className="relative min-w-full">
                                ...
                                <button
                                  onClick={() =>
                                    removeContent(keyIndex, contentIndex)
                                  }
                  ...
                  <div className="border flex mt-10 items-center justify-center border-[#898787F4] border-dashed h-[150px] relative rounded-[10px] p-10">
                    <div className="absolute top-[-17px] left-[150px] border bg-white border-[#f4f4f4]/95 rounded-full py-[5px] px-[30px] text-sm font-clashmd">
                      Key
                    </div>
                    <button
          ...
          {isAdd ? (
            <button
              ...
              disabled={onSubmitLoading2}
              className="py-[15px] hover:bg-white hover:text-primary transition-all duration-150 border-2 border-primary w-fit mx-auto px-20 rounded-full bg-primary text-white font-clashmd"
            >
              {onSubmitLoading2 ? (
                <svg
                  className="h-5 w-5 animate-spin"
            ...
            <button
              ...
              className="py-[15px] hover:bg-white hover:text-primary transition-all duration-150 border-2 border-primary w-fit mx-auto px-[50px] rounded-full bg-primary text-white font-clashmd"
            >
              Add a new service
            </button>
          )}
